...
int CcontrolSerialNetPortDlg::explainOmissionDetection(CStdioFile* file, int workNum, bool* Continue)
{
	...
	for (int i = 0; i < 9; i++)
	{
		comm[i] = ChineseTransformation(MiddleComm, file);//0 ABJG?  1路径 2标号
	}

	CString number = explainParameter(_T("Number"), comm[2]);

	// CString ROIX = explainParameter(_T("模式识别中的ROIX"), comm[0]);
	...
	int res = imageProcess(&DigUserHookData[workNum], &CPublic::g_imagePosInfo1[workNum][(CPublic::g_imagePara1[workNum].proNum - 1) % PRONUM1], workNum, 2);//---------------（1.2.1.5.1.1）获取图像处理结果函数
...
int CcontrolSerialNetPortDlg::explainImageProcessing1(CStdioFile* file, int workNum, bool* Continue)//---------------------------（1.2.1.5.1）模板匹配图像处理解析函数
{
	...
	int res = imageProcess(&DigUserHookData[workNum], &CPublic::g_imagePosInfo1[workNum][(CPublic::g_imagePara1[workNum].proNum - 1) % PRONUM1], workNum, 0);//---------------（1.2.1.5.1.1）获取图像处理结果函数
...
std::map<std::string, cv::Scalar> create_class_colors(const std::vector<std::string>& classes)
{
	...
	cv::RNG rng(12345);  // 固定随机种子，以便每次颜色都一致
...
void file_iterator(DCSP_CORE*& p, const std::string& img_path, const std::string& output_path, int a)
{
	...
	if (a == 0)
	{
		// 场景1判断：目标1,2,3,4必须存在，目标5,6,7,8,9,10必须不存在
		...
		{
			mainDlg->AddString(_T("abjg错漏装检测合格"));
		}
		else if (scenario1_pass = false)
		{
			mainDlg->AddString(_T("abjg错漏装检测不合格"));
		}

	}
	else if (a == 1)
	{
		// 场景2判断：目标9必须存在，目标1,2,3,4,5,6,7,8,10必须不存在
		...
		{
			mainDlg->AddString(_T("wlg错漏装检测合格"));
		}
		else if (scenario2_pass = false)
		{
			mainDlg->AddString(_T("wlg错漏装检测不合格"));
		}
	}
	...
	if (!cv::imwrite(output_path, img)) {
		mainDlg->AddString(_T("检测结果图像保存失败"));
	}
	else {
		mainDlg->AddString(_T("检测结果图像保存成功"));
	}
}
...
int OMprocess304(MIL_ID MilSystem, MIL_ID MilImage, MIL_ID MilOverlayImage, int workNum, PositionInfo* output)
{
	...
	CStringW cstrW1(imgpath_or); // 转换为 Unicode CString
	std::wstring stdWStr1 = cstrW1.GetString(); // 获取 std::wstring
	std::string stdimgpath_or(std::begin(stdWStr1), std::end(stdWStr1)); // 转换为 std::string

	CStringW cstrW2(model_path); // 转换为 Unicode CString
	std::wstring stdWStr2 = cstrW2.GetString(); // 获取 std::wstring
	std::string stdmodel_path(std::begin(stdWStr2), std::end(stdWStr2)); // 转换为 std::string

	CStringW cstrW3(imgpath_om); // 转换为 Unicode CString
	std::wstring stdWStr3 = cstrW3.GetString(); // 获取 std::wstring
	std::string stdimgpath_om(std::begin(stdWStr3), std::end(stdWStr3)); // 转换为 std::string
	...
	if (UserHookDataPtr->GrabInProgress == M_TRUE)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					MbufCopy(ModifiedBufferId, UserHookDataPtr->MilImageDisp);
					mainDlg->AddString(_T("错漏装检测执行完毕"));
				}
				else
				{
					mainDlg->AddString(_T("错漏装检测执行失败"));
				}
